...
public class MP7CSearcher implements MelampoSearcher {
	...
	private static SAPIRObjectFieldAdder m_sfaALL;
	...
	public void OpenIndex(File propertyFile) throws IOException, VIRException {
		...
		MP7CSettings settings = new MP7CSettings(propertyFile);

		m_soSearcher = new SapirSearcher(settings);

		m_sreader = new SapirReader(settings);
		//m_so = m_sreader.getAllSapirObject();

		String indexPath = settings.getLuceneIndexPathMPG7C();
		if (indexPath != null) {
		...
		m_wrapper.addAnalyzer(Parameters.MP7ALL, analyzer);
