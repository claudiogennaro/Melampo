...
public class CreateSapirPivots {
	...
	public static void main(String[] args) {
		try {
			...
			CreateSapirPivots csp = new CreateSapirPivots();
	...
	public void LoadDataset(SapirReader reader){
		m_so = new SAPIRObject[reader.size()];
		
		for(int i=0;i<reader.size();i++){
			try {
				m_so[i] = reader.getSapirObject(i);
	...
	public void StorePivots(String path, SapirReader reader) {
        try{
            ...
            for(int i=0;i<m_rind.size();i++){
            	SAPIRObject s = reader.getSapirObject(m_rind.get(i));
            	s.writeData(pivFile);
